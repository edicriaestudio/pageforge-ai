...
import { LayoutDashboard, FolderKanban, Settings, Zap, LogOut, ChevronLeft, ChevronRight, LayoutTemplate, Blocks } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-zinc-300 selection:bg-emerald-500/30 overflow-hidden font-sans">
      ...
      <aside className={`${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-64 opacity-100'} border-r border-zinc-900 bg-black flex flex-col relative z-20 transition-all duration-300 ease-in-out shrink-0`}>
        ...
        <div className="absolute top-0 left-0 w-full h-32 bg-emerald-900/10 blur-[50px] pointer-events-none" />
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <Link href="/dashboard" className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${pathname === '/dashboard' ? 'bg-zinc-900/50 text-emerald-400 border border-zinc-800/50' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50'}`}>
            <LayoutDashboard size={18} className="shrink-0" />
            <span className="font-medium text-sm whitespace-nowrap">Visão Geral</span>
          </Link>
          
          <Link href="/dashboard/projects" className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${pathname.includes('/projects') ? 'bg-zinc-900/50 text-emerald-400 border border-zinc-800/50' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50'}`}>
          ...
          <Link href="/dashboard/templates" className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${pathname.includes('/templates') ? 'bg-zinc-900/50 text-emerald-400 border border-zinc-800/50' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50'}`}>
            <LayoutTemplate size={18} className="shrink-0" />
            <span className="font-medium text-sm whitespace-nowrap">Templates</span>
          </Link>

          <Link href="/dashboard/assets" className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${pathname.includes('/assets') ? 'bg-zinc-900/50 text-emerald-400 border border-zinc-800/50' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50'}`}>
          ...
          <Link href="/dashboard/settings" className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${pathname.includes('/settings') ? 'bg-zinc-900/50 text-emerald-400 border border-zinc-800/50' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50'}`}>
      ...
      <main className="flex-1 flex flex-col relative overflow-y-auto bg-black">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-900/5 blur-[100px] pointer-events-none rounded-full" />
